...
const ItemCustomerWiseSalesPage = () => {
  ...
  const hasValidRange = Boolean(
    selectedItem?.value && selectedCustomer?.value && dateRange.start && dateRange.end
  );
  ...
  const salesQuery = useQuery({
    queryKey: ["item-customer-wise-sales", selectedItem?.value, selectedCustomer?.value, dateRange.start, dateRange.end],
  ...
  const handlePrint = () => {
    ...
    printDocument.open();
    printDocument.write(
      `<!doctype html><html><head><meta charset="utf-8" /><title>Item-Customer-Wise Sales</title>${reportPrintStyles}</head><body><div id="print-root"></div></body></html>`
    );
    printDocument.close();
  ...
  const handleExport = () => {
    const header = ["S/N", "Customer", "Invoice No.", "Invoice Date", "Base Unit", "Quantity", "Rate", "Discount %", "Tax %", "Value"];
    ...
    link.download = `item-customer-wise-sales-${new Date().toISOString().split("T")[0]}.csv`;
    link.click();
  };

  return (
    <SectionCard
      title="Item-Customer-Wise Sales"
      description="View item-level sales details with pricing information for a specific item and customer."
    >
      <div className="space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-4">
          ...
          <FormField label="Customer" required description="Select a customer.">
            <SearchSelect
          ...
          <>
            {salesQuery.isLoading || salesQuery.isFetching ? (
              <div className="text-center py-12">
                <p className="text-slate-600">Loading item-customer-wise sales…</p>
              </div>
            ) : salesQuery.isError ? (
              <div className="text-center py-12">
                  ...
                  <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-slate-200">
                      ...
                      <tfoot className="bg-gradient-to-r from-slate-100 to-slate-50">
                        <tr>
                          <td colSpan="5" className="px-4 py-4 text-sm font-bold text-slate-800">
                            Totals
                          </td>
...
export default ItemCustomerWiseSalesPage;
