...
const DateWiseDamageInSummaryPage = () => {
  ...
  const reportQuery = useQuery({
    queryKey: ["date-wise-damage-in", dateRange.start, dateRange.end],
    enabled: hasValidRange,
    queryFn: async () => {
      const response = await api.get("/reports/sales/date-wise-damage-in", {
        params: {
  ...
  const handlePrint = () => {
    ...
    printDocument.open();
    printDocument.write(
      `<!doctype html><html><head><meta charset="utf-8" /><title>Date Wise Damage In Summary</title>${reportPrintStyles}</head><body><div id="print-root"></div></body></html>`
    );
    printDocument.close();
    ...
    const mountContent = () => {
      ...
      titleHeading.textContent = "DATE WISE DAMAGE IN SUMMARY";
  ...
  const handleExport = () => {
    ...
    link.download = `damage-in-summary-${new Date().toISOString().split("T")[0]}.csv`;
    link.click();
  };
  ...
  return (
    <SectionCard
      title="Date Wise Damage In Summary"
      description="View damage in items by date range."
    >
      <div className="space-y-6">
        ...
        {rows.length === 0 && !reportQuery.isLoading && hasValidRange && (
          <div className="rounded-lg border border-yellow-200 bg-yellow-50 p-4 text-center text-sm text-yellow-700">
            No damage in records found for the selected date range.
          </div>
        )}
...
export default DateWiseDamageInSummaryPage;
