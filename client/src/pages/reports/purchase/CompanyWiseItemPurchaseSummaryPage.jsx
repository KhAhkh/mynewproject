...
const CompanyWiseItemPurchaseSummaryPage = () => {
  const [filters, setFilters] = useState(defaultFilters);
  const [companySearch, setCompanySearch] = useState("");

  const companyLookup = useQuery({
    queryKey: ["company-wise-item-purchase-companies", companySearch],
    queryFn: async () => {
  ...
  const reportQuery = useQuery({
    queryKey: ["purchase/company-wise-item-summary", filters],
    enabled: Boolean(filters.companyId),
    queryFn: async ({ queryKey }) => {
      const [, params] = queryKey;
      const response = await api.get("/reports/purchase/company-wise-item-summary", {
        params: {
  ...
  return (
    <SectionCard
      ...
      description="Select a company and date range to view summary of items purchased from that company."
      actions={
        ...
        <div className="space-y-6">
          {!filters.companyId ? (
            <div className="rounded-lg border border-slate-200 bg-slate-50 p-6 text-center text-slate-500">
              Select a company to view purchase summary.
            </div>
...
export default CompanyWiseItemPurchaseSummaryPage;
