...
const SalesmanSalesSummaryPage = () => {
  ...
  const hasValidRange = Boolean(selectedSalesman?.value && dateRange.start && dateRange.end);

  const salesmanDirectory = useQuery({
    queryKey: ["salesmen-directory", salesmanSearch],
    staleTime: 0,
    queryFn: async () => {
  ...
  const salesQuery = useQuery({
    queryKey: ["salesman-sales-summary", selectedSalesman?.value, dateRange.start, dateRange.end],
  ...
  const totals = salesQuery.data?.totals ?? { totalAmount: 0, count: 0, amountPaid: 0, outstanding: 0 };
  ...
  const handlePrint = () => {
    ...
    printDocument.open();
    printDocument.write(
      `<!doctype html><html><head><meta charset="utf-8" /><title>Salesman Sales Summary</title>${reportPrintStyles}</head><body><div id="print-root"></div></body></html>`
    );
    printDocument.close();
  ...
  const handleExport = () => {
    const header = ["S/N", "Invoice No.", "Invoice Date", "Amount", "Paid", "Outstanding"];
    const dataRows = rows.map((row, idx) => [
      idx + 1,
      row.invoiceNo || "",
      row.invoiceDate || "",
      Number(row.totalAmount || 0).toFixed(2),
      Number(row.amountPaid || 0).toFixed(2),
    ...
    dataRows.push([
      ...
      Number(totals.amountPaid).toFixed(2),
    ...
    link.download = `salesman-sales-summary-${new Date().toISOString().split("T")[0]}.csv`;
    link.click();
  };
  ...
  return (
    <SectionCard
      ...
      description="View invoice-level sales for a specific salesman within a date range."
    >
      <div className="space-y-6">
        {/* Filters */}
        <div className="grid gap-4 md:grid-cols-3">
          <FormField label="Salesman" required description="Select a salesman to view their sales.">
            <SearchSelect
          ...
          <>
            {salesQuery.isLoading || salesQuery.isFetching ? (
              <div className="text-center py-12">
                <p className="text-slate-600">Loading salesman sales summary…</p>
              </div>
            ) : salesQuery.isError ? (
              <div className="text-center py-12">
              ...
              <>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
                  <div className="bg-gradient-to-br from-indigo-50 to-indigo-100 rounded-lg p-4 border border-indigo-200">
                    <p className="text-xs text-indigo-700 uppercase font-medium">Total Invoices</p>
                  ...
                  <div className="bg-gradient-to-br from-blue-50 to-blue-100 rounded-lg p-4 border border-blue-200">
                    ...
                    <p className="text-2xl font-bold text-blue-900 mt-1">{formatCurrency(totals.totalAmount)}</p>
                  </div>
                  <div className="bg-gradient-to-br from-emerald-50 to-emerald-100 rounded-lg p-4 border border-emerald-200">
                    <p className="text-xs text-emerald-700 uppercase font-medium">Total Paid</p>
                    <p className="text-2xl font-bold text-emerald-900 mt-1">{formatCurrency(totals.amountPaid)}</p>
                  </div>
...
export default SalesmanSalesSummaryPage;
